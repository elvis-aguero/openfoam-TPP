...
def run_postprocess_oscar(case_name, action):
    ...
    header = [
        ...
        "# We use a dedicated venv for compute nodes to handle Python version differences",
        "# (e.g., Login=Py3.13 vs Compute=Py3.9) without breaking the local setup.",
        "COMPUTE_VENV=\"sloshing_compute\"",
        "",
        "if [ ! -d \"$COMPUTE_VENV\" ]; then",
        "  echo \"📦 First run detected. Building persistent compute environment in $COMPUTE_VENV...\"",
        "  python3 -m venv $COMPUTE_VENV",
        "  source $COMPUTE_VENV/bin/activate",
        "  # Upgrade pip first to avoid issues",
        "  pip install --upgrade pip",
        "  pip install -r requirements.txt",
        "else",
        "  echo \"✅ Using existing compute environment: $COMPUTE_VENV\"",
        "  source $COMPUTE_VENV/bin/activate",
        "fi",
        "# --------------------------------------",
        "",
