...
def run_postprocess_oscar(case_name, action):
    ...
    header = [
        ...
        "set -euo pipefail",
        "",
        "# --- Auto-Repair Venv Mismatch ---",
        "# Detect if the existing venv was built with a different python version",
        "if [ -d \"sloshing\" ]; then",
        "  VENV_PY=$(ls sloshing/lib/ | grep python || true)",
        "  NODE_PY=$(python3 -c \"import sys; print(f'python{sys.version_info.major}.{sys.version_info.minor}')\")",
        "  if [[ \"$VENV_PY\" != \"$NODE_PY\" && -n \"$VENV_PY\" ]]; then",
        "    echo \"⚠️  Venv mismatch detected (Venv: $VENV_PY vs Node: $NODE_PY). Rebuilding...\"",
        "    rm -rf sloshing",
        "  fi",
        "fi",
        "# ---------------------------------",
