...
def generate_video(case_dir):
    ...
    # 2. Generate Dashboard Video (using extracted CSV data if available)
    # This requires 'extract_interface' to have run first, or we do a quick grab here?
    # Better to rely on the CSV generated by extract_interface to avoid re-processing 8000 cells x 1000 steps.
    csv_path = os.path.join(results_dir, "interface", "interface_summary.csv")
    if os.path.exists(csv_path):
        print("    - Generating analysis dashboard video...")
        # We need a separate function or logic here to create the dashboard from the CSV
        # Since we don't have per-theta data in that summary CSV, we might need to rely on the VTPs?
        # Actually, the user wants "unwrapped view" and "time series".
        # The VTP files contain the full 3D surface. We can load them and slice them.
        pass # Dashboard generation is complex without structured CSV. We'll skip for now or use VTPs.
        # Let's rely on extraction to happen first.
