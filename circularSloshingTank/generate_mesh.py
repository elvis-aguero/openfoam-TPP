...
(
    ...
    {{
        name    atmosphereFaces;
        type    faceSet;
        action  new;
        source  boxToFace;
        box     ({xmin} {ymin} {z_face_min}) ({xmax} {ymax} {z_face_max});
    }}
);

// ************************************************************************* //
""".format(xmin=xmin, ymin=ymin, zmin=zmin, xmax=xmax, ymax=ymax, zmax=zmax, z_face_min=z_face_min, z_face_max=z_face_max)
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)


def _write_create_patch_dict(path):
    content = """/*--------------------------------*- C++ -*----------------------------------*\\
  =========                 |
  \\      /  F ield         | OpenFOAM: The Open Source CFD Toolbox
   \\    /   O peration     | Website:  https://openfoam.org
...
if __name__ == "__main__":
    ...
    if mesher == "snappy":
        ...
        _write_create_patch_dict(os.path.join("system", "createPatchDict"))
